  ...
  <div class="whole_box">
    <div class="up_box">
      ...
      <div class="person_box" style="margin: 0 auto">
        <el-avatar style="width: 100px;height: 100px;margin-left: 30px;float:left" :src="this.personInfo.avatar_image"></el-avatar>
        <div class="user_box" style="float: left;height: 100px;margin-left: 30px"><span style="position:relative;top: 50%">用户名：{{ this.personInfo.username }}</span></div>
        <div class="user_box" style="float: left;height: 100px;margin-left: 30px"><span style="position:relative;top: 50%">邮箱：{{this.personInfo.email}}</span></div>
        <div class="user_box" style="float: left;height: 100px;margin-left: 30px"><span style="position:relative;top: 50%">手机号码：{{this.personInfo.phone}}</span></div>
        <div class="user_box" style="float: left;height: 100px;margin-left: 30px"> <el-button style="position:relative;top: 46%" @click="openEditDiag"><i class="el-icon-edit">修改个人信息</i></el-button></div>
        <div class="user_box" style="float: left;height: 100px;margin-left: 30px"> <el-button style="position:relative;top: 46%" @click="openChangePassDiag" type="danger"><i class="el-icon-edit">修改密码</i></el-button></div>
<!--        <el-upload-->
<!--            list-type="text"-->
...
<!--        </el-upload>-->
    ...
    <div class="down_box">
      ...
      <el-divider></el-divider>
      <el-row>
        <el-col   :xs="14" :sm="14" :md="14" :lg="8" :xl="6" :span="6" v-for="(item,index)  in historyLesson" :key="index">
    ...
    >
      <el-form :model="changePassInfo" :rules='rules2' ref='changePassInfo'>
        <el-form-item label="旧密码" :label-width="formLabelWidth" prop='old_password'>
          <el-input v-model="changePassInfo.old_password" type="password" style='width: 90%' placeholder="请输入旧密码"></el-input>
        </el-form-item>
        <el-form-item label="新密码" :label-width="formLabelWidth" prop='new_password'>
          <el-input v-model="changePassInfo.new_password" style='width: 90%' type="password" placeholder="请输入新密码"></el-input>
        </el-form-item>
        <el-form-item label="重复输入新密码" :label-width="formLabelWidth" prop='repeat_new_password'>
          <el-input v-model="changePassInfo.repeat_new_password" style='width: 90%' type="password" placeholder="请再次输入新密码"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods:{
    async getHistoryLesson(){
      let path = this.$root.URL+"/frontUser/getHistoryLesson?uid="+localStorage.getItem("user_id");
    ...
    enterChangePass(formName){
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          setTimeout(async () => {
            console.log(JSON.stringify(this.changePassInfo))
            this.changePassInfo.uid = localStorage.getItem("user_id");
            let path = this.$root.URL+'/frontUser/changePass';
            if(this.changePassInfo.new_password != this.changePassInfo.repeat_new_password){
              this.$message.error("两次密码输入不一致，请重新输入！")
            }else {
              await post(path, this.changePassInfo).then(res => {
    ...
    async getInitData(){
      ...
      let path = this.$root.URL+"/frontUser/getPersonInfo?uid="+localStorage.getItem("user_id");
      await get(path).then(res=>{
        this.personInfo.username = res.data.data.username;
        this.personInfo.email = res.data.data.email;
        this.personInfo.phone = res.data.data.phone;
        this.personInfo.avatar_image = res.data.data.avatar_image;
        this.edit_personInfo.avatar_image = res.data.data.avatar_image;
        this.edit_personInfo.email = res.data.data.email;
        this.edit_personInfo.phone = res.data.data.phone;
        this.fileListFront.push({name:'xxx',url:this.personInfo.avatar_image.split(",")[0]})   //在el-upload中回显后台返回的地址
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          setTimeout(async () => {
            ...
            this.edit_personInfo.uid = localStorage.getItem("user_id");
            let path = this.$root.URL+'/frontUser/updatePersonInfo';
            await post(path, this.edit_personInfo).then(res => {
              console.log(res.data);
              if (res.data.code === '200') {
